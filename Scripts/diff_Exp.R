...
colnames(all_counts_combined)[1:12]=c(paste("WT",1:6,sep="_"),paste("AHRKO",1:6,sep="_"))

all_counts_combined=all_counts_combined[1:(nrow(all_counts_combined)-5),]

write.table(all_counts_combined,file="CR705_WT_AHRKO_Counts_File.txt",col.names = T,row.names = T,sep="\t",quote = F)


condition_df=as.data.frame(c(rep("WT",6),rep("AHRKO",6)))
...
library(DESeq2)
library(edgeR)
dds = DESeqDataSetFromMatrix(countData = all_counts_combined[,c(1:12)],
...
mdsplot = plotPCA(vsd, intgroup = c("condition")) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank(), axis.line = element_line(colour = "black"))

pdf(file="CR705_MDS_plot.pdf",width = 6,height=6)
mdsplot
dev.off()
...
mdsplot=ggplot(mds, aes(x = `1`, y = `2`, color = condition)) +
  geom_point(size = 3) + coord_fixed() + ggtitle("MDS Plot")+xlab("Leading lgFC dim 1 (40%)")+ylab("Leading lgFC dim 1 (15%)") + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_blank(), axis.line = element_line(colour = "black"))
